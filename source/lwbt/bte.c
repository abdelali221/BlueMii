...
static err_t bte_process_input(void *arg,struct l2cap_pcb *pcb,struct pbuf *p,err_t err)
{
	...
	if (pcb->psm == SDP_PSM)
	{
		if (bte->recv) bte->recv(bte->cbarg,buf,len);
	}
	else
	{
		len--;
...
s32 bte_connectasync(struct bte_pcb *pcb,struct bd_addr *bdaddr,s32 (*conn_cfm)(void *arg,struct bte_pcb *pcb,u8 err))
{
	...
	err = l2ca_connect_req(l2capcb,bdaddr,HIDP_PSM,HCI_ALLOW_ROLE_SWITCH,l2cap_connected);
	if(err!=ERR_OK) {
		l2cap_close(l2capcb);
		err = ERR_CONN;
	}
...
s32 bte_connectasync_step2(struct bte_pcb *pcb,s32 (*conn_cfm)(void *arg,struct bte_pcb *pcb,u8 err))
{
	...
	err = l2ca_connect_req(l2capcb,&(pcb->bdaddr),INTR_PSM,HCI_ALLOW_ROLE_SWITCH,l2cap_connected);
	if(err!=ERR_OK) {
		l2cap_close(l2capcb);
		err = ERR_CONN;
	}
...
s32 bte_listenasync(struct bte_pcb *pcb,struct bd_addr *bdaddr,s32 (*conn_cfm)(void *arg,struct bte_pcb *pcb,u8 err))
{
	...
	err = l2cap_connect_ind(l2capcb,bdaddr,SDP_PSM,l2cap_accepted);
	if(err!=ERR_OK) {
		l2cap_close(l2capcb);
		err = ERR_CONN;
	}
	...
	err = l2cap_connect_ind(l2capcb,bdaddr,HIDP_PSM,l2cap_accepted);
	if(err!=ERR_OK) {
		l2cap_close(l2capcb);
		err = ERR_CONN;
	}
...
s32 bte_listenasync_step2(struct bte_pcb *pcb,s32 (*conn_cfm)(void *arg,struct bte_pcb *pcb,u8 err))
{
	...
	err = l2cap_connect_ind(l2capcb,&(pcb->bdaddr),INTR_PSM,l2cap_accepted);
	if(err!=ERR_OK) {
		l2cap_close(l2capcb);
		err = ERR_CONN;
	}
...
s32 bte_sendsdp(struct bte_pcb *pcb,void *message,u16 len)
{
	err_t err;
	struct pbuf *p;
	...
	if((p=btpbuf_alloc(PBUF_RAW,(len),PBUF_RAM))==NULL) {
		ERROR("bte_sendsdp: Could not allocate memory for pbuf\n");
		return ERR_MEM;
	}

	memcpy(p->payload,message,len);

	err = l2ca_datawrite(pcb->sdp_pcb,p);
	btpbuf_free(p);
